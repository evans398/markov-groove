...
function reset() {
    ...
    // post("Second Order Markov Duration Model Reset\n");
}

function followExternal() {
    listenToExternalInput = true;
    // post("Second Order Markov Duration Model following external input\n");
}

function followInternal() {
    listenToExternalInput = false;
    // post("Second Order Markov Duration Model following internal state\n");
}

function dump() {
    // post("Second Order Duration Counts: " + JSON.stringify(transitionCounts) + "\n");
    // post("Second Order Duration Probabilities: " + JSON.stringify(transitionProbs) + "\n");
    outlet(1, "Second Order Duration Counts: " + JSON.stringify(transitionCounts));
    outlet(2, "Second Order Duration Probabilities: " + JSON.stringify(transitionProbs));
}
